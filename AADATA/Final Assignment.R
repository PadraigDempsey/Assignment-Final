...
ggplot(high_child_deprivation, aes(x = reorder(country, child), y = child, fill = country)) +
  ...
  guides(fill = guide_legend(title = "Country")) # Change the legend title
...
ggplot(high_child_deprivation, aes(x = reorder(country, child), y = child)) +
  ...
  coord_flip() +
  scale_fill_gradient(low = "lightcoral", high = "darkred") +  # Gradient from light to dark red
...
# Save the plot with correct theme settings
...
#ScatterPlot1 
     ggplot(merged_data_1, aes(x = gdp, y = child)) +
      ...
      geom_smooth(method = "lm", color = "white", se = FALSE) + # Add a linear regression line
      theme_minimal() + 
      custom_theme +
